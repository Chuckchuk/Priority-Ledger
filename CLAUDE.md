...
- **State** lives in one in-memory object: `state = { location, tasks: [], days: [], categories: [] }`. Everything persists as one JSON blob under a single storage key (`tracker-state`) — don't split it into multiple keys.
