...
const CATEGORY_PALETTE_SETS = {
  ...
  // instead). Only 9 colors each rather than the 12 every light set
  // uses — plenty for a real hue sweep without the extra authoring/
  // upkeep a full dozen would need per mood. Deliberately more saturated
  // than the light sets' own colors: a muted tone that reads fine
  // against cream paper goes flat/lifeless against a near-black one, so
  // these lean brighter/more vivid to actually pop there. midnight =
  // cool (blue/teal/indigo/violet), ember = warm (red/rust/amber/olive),
  // noir = near-monochrome grays/taupes/slates (the dark counterpart to
  // 'greyscale' above) — same three-way split in mood 'classic'/
  // 'greyscale'/'pastel' already use, just for dark surfaces.
  midnight: {
    id: 'midnight', label: 'Midnight',
    colors: ['#3FBDB0','#5AC79A','#5CACD9','#6F93D9','#8B7FE0','#A87DE0','#C77DD1','#D97C9E','#E0A458']
  },
  ember: {
    id: 'ember', label: 'Ember',
    colors: ['#C15A5A','#C97244','#D9895A','#E0A458','#C6B15A','#9BAF5E','#C97D6E','#B1546E','#A6763E']
  },
  noir: {
    id: 'noir', label: 'Noir',
    colors: ['#E0E0E0','#C0C0C0','#A0A0A0','#808080','#D8CDBE','#B0A38F','#8C7F6C','#A8ADB4','#767C83']
  }
};
