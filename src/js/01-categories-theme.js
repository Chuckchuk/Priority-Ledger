...
const CATEGORY_PALETTE_SETS = {
  ...
  // mixed mid-to-dark tones 'classic'/'noir' both use. Darkened a slight
  // -7% (shadeHex(hex, -0.07), the same multiplicative scaling every
  // other darken/lighten in this app uses) across the board per the
  // project owner's own callout that the tab bar's own pastel colors
  // read a little washed-out/mismatched against most of the actual
  // Pastel Desk & Ledger/UI presets — deliberately a much smaller nudge
  // than Ember's own below, since "pastel" is the whole point of this
  // set and it only needed a slight correction, not a real re-tone.
  pastel: {
    id: 'pastel', label: 'Pastel',
    colors: ['#D88E83','#DFA671','#DFCD79','#BBCC82','#9CC99C','#93CCBA','#89BBC1','#8CB1D0','#98A2D0','#B59CD0','#D09CBE','#D8A2AA']
  },
  ...
  // The project owner explicitly singled this set out as the one part of
  // Ember they actually like — the Desk & Ledger and UI Colors sets for
  // Ember were both overhauled into an autumnal mood instead; this kept
  // its exact hues by contrast. Later darkened a uniform -18%
  // (shadeHex(hex, -0.18)) across the board, per a separate callout that
  // the set read "a little bright" for a tab/dot color once seen next to
  // the rest of the app — same multiplicative darken every shade/tint in
  // this app already uses, so hue and relative saturation are unchanged,
  // just value. Original (pre-darken) hues, for reference:
  // ['#C15A5A','#C97D6E','#C97244','#D9895A','#B8672E','#A6763E',
  // '#E0A458','#C6B15A','#9BAF5E','#7A8C4A','#B1546E','#8C3E42']. All 9
  // of THOSE were the exact original colors from the first pass; 3 new
  // ones (a deeper terracotta,
  // a true olive-green, and an oxblood/maroon) filled out a 12th-color hue
  ...
  ember: {
    ...
    colors: ['#9E4A4A','#A5675A','#A55D38','#B2704A','#975426','#886133','#B88648','#A2914A','#7F904D','#64733D','#91455A','#733336']
  },
  // Was two separate sets ('noir', a near-black-to-mid-gray register built
  // for a dark card, and 'greyscale', a lighter washed-out register built
  // for light paper) — consolidated into one per the project owner's own
  // ask, since a category's own palette is picked independently of the
  // Desk & Ledger one anyway (nothing stops picking "Noir" categories over
  // a light-paper desk, or vice versa), so keeping two separate near-
  // monochrome sets split by which paper they'd been tuned against never
  // actually lined up with how they're chosen in practice. Rebuilt as one
  // genuinely mixed 12, not just the old 'noir' with 'greyscale' spliced
  // in — every entry here sits in a "safe middle" lightness band (roughly
  // 0.38–0.59 relative luminance) so it reads with reasonable contrast
  // against EITHER a near-white or a near-black card, the actual
  // constraint a single merged set has to satisfy that neither original
  // one did alone. Kept in the same three-group shape both predecessors
  // already used (warm taupe / neutral grey / cool slate), each group
  // dark-to-light, with two brand-new entries per group filling out a
  // real light-to-dark range within the safe band instead of just
  // trimming the old 24 candidates down to 12.
  noir: {
    id: 'noir', label: 'Noir',
    colors: ['#6E6052','#7A6C58','#8C7F6C','#A0907A','#606060','#707070','#808080','#909090','#5E6570','#767C83','#8B9198','#9098A0']
  }
};
...
const UI_COLOR_PRESET_SETS = {
  ...
  // Midnight/Ember/Noir — see CATEGORY_PALETTE_SETS' own comment above for
  // the mood split. These need to read as a small icon/text color against
  // EITHER a light or dark card (Ember now spans both, see its own
  // comment below) — mid-value, richly saturated jewel tones balance
  // reasonably against both extremes, the same way Classic's own colors
  // already do in light mode. They no longer also have to double as a
  // solid button background with a FIXED light text color on top (a light
  // ink hardcoded for dark mode used to sit on these directly, e.g.
  // .locbadge/.addbtn) — that pairing is what originally forced these
  // values to stay mid-dark rather than genuinely bright, since a bright
  // accent has great contrast as an icon but terrible contrast behind
  // fixed light text. --on-primary (applyThemeObject() below) now derives
  // that text color from each accent's own luminance instead, so this
  // constraint is gone — kept these mid-value anyway since that's still
  // the right register for an icon color, not because the button-text
  // math still demands it.
  ...
  // Overhauled into a genuinely autumnal mood, per the project owner's own
  // ask — the previous set (Rust & Gold, Ember & Clay, Wine & Rose, Copper
  // & Olive, Brick & Brass) wasn't disliked for its hues so much as for
  // reading as generic "warm dark mode" rather than actually autumnal;
  // these pairs are pulled from the same warm hue families
  // CATEGORY_PALETTE_SETS.ember's own liked colors already use (the one
  // part of Ember the project owner explicitly kept), so picking Ember
  // here and for Category Colors reads as one coordinated look. Built to
  // pair with DESK_PAPER_PRESET_SETS.ember's own new light-AND-dark
  // autumnal presets (see that set's comment) rather than assuming a dark
  // card the way the old set did — see the comment above this whole block
  // for why these no longer have to stay artificially mid-dark to survive
  // as a button background. Ordered as its own warm sweep (bright cider-
  // red through pumpkin/gold/bark/rust, wrapping to a deeper maple-maroon
  // at the end) mirroring how CATEGORY_PALETTE_SETS.ember itself wraps to
  // wine/maroon at its own end.
  ember: {
    id: 'ember', label: 'Ember',
    presets: [
      { id:'ciderclay',   label:'Cider & Clay',    primary:'#8C3E42', primaryLight:'#B1545A', secondary:'#D9895A', secondaryLight:'#EDAD82' },
      { id:'pumpkingold', label:'Pumpkin & Gold',  primary:'#B8672E', primaryLight:'#D68A4E', secondary:'#C99A4E', secondaryLight:'#E6BC72' },
      { id:'harvestbark', label:'Harvest & Bark',  primary:'#A6763E', primaryLight:'#C99A5E', secondary:'#4A3320', secondaryLight:'#6B4C32' },
      { id:'rustwheat',   label:'Rust & Wheat',    primary:'#9C4530', primaryLight:'#C3563C', secondary:'#C6B15A', secondaryLight:'#DED085' },
      { id:'maplemoss',   label:'Maple & Moss',    primary:'#7A2E24', primaryLight:'#A04336', secondary:'#7A8C4A', secondaryLight:'#9DB06C' }
    ]
  },
  // Consolidated from the old 'noir' (near-neutral greys/slates, above)
  ...
  noir: {
    ...
    presets: [
      ...
      { id:'graphitebone',  label:'Graphite & Bone',  primary:'#3A3A3A', primaryLight:'#525252', secondary:'#B5AC96', secondaryLight:'#CFC7B0' },
      ...
      { id:'ashivory',      label:'Ash & Ivory',      primary:'#7A756C', primaryLight:'#948E83', secondary:'#C7BFAE', secondaryLight:'#DDD6C7' }
    ]
  }
};
...
// every Classic/Noir entry shares — a soft dusty mid-tone desk
...
const DESK_PAPER_PRESET_SETS = {
  ...
  // Dark presets in the sets below (Midnight fully, Ember/Noir partly —
  // see their own comments) are the real enabler of an actual dark
  // *mode* (not just another moody desk under the same always-light
  // paper the sets above use): paper itself goes dark, for the first
  // time — applyThemeObject() already derives --ink/--ink-soft/--line
  // from `relLuminance(t.paper) < 0.5` (see its own comment, further down
  // this file) and flips to a light, legible ink automatically whenever
  // that's true, so a dark paper "just works" through the exact same
  // mechanism a custom dark pick already would have. paper stays only a
  // little lighter than its own bg in every dark entry (unlike every
  // light entry's huge bg/paper contrast) — enough for the card to still
  // visibly sit on the desk as its own surface, without reading as a
  // stray light patch against an otherwise dark page.
  midnight: {
    id: 'midnight', label: 'Midnight',
    presets: [
      ...
      // Moved over from Ember per the project owner's own ask — Ember's
      // own dark presets were overhauled into a fully autumnal mood (see
      // that set's comment below) that this deep wine/plum pairing no
      // longer fit, but they explicitly still liked it on its own merits,
      // and its cool-leaning wine/plum hue sits comfortably next to
      // Twilight & Plum's own violet here. Appended rather than re-sorted
      // into the cool blue→green→violet sweep above — a genuine "warm
      // outlier" addition, same idea as Driftwood's own outlier spot in
      // the old Greyscale set.
      { id:'midnightwine',   label:'Midnight & Wine',     bg:'#170A0D', paper:'#2A131A' }
    ]
  },
  // Overhauled into a genuinely autumnal mood spanning BOTH light and
  // dark, per the project owner's own ask — the previous set (Char &
  // Ember, Coal & Rust, Smolder & Umber, Midnight & Wine, Cocoa & Copper)
  // was five near-identical near-black-brown dark presets and nothing
  // else, which read as "generic warm dark mode" rather than actually
  // autumnal; Midnight & Wine (the one the project owner did like) moved
  // to Midnight above instead of surviving here. Three light presets
  // (acorn-gold/pumpkin/maple, under warm cream-to-parchment paper) plus
  // three dark ones (the same hues carried into a near-black register,
  // paper only a little lighter than bg like every dark preset here) —
  // the first Desk & Ledger set in the app to genuinely span both
  // registers under one name rather than being purely light (Classic/
  // Pastel) or purely dark (Midnight). Paired with
  // UI_COLOR_PRESET_SETS.ember's own new autumnal presets (matching
  // label pairs — Pumpkin & Cream desk ↔ Pumpkin & Gold UI, etc. — though
  // nothing enforces picking them together). Acorn & Wheat replaced an
  // original "Harvest & Birch" light preset the project owner didn't
  // like on sight — a richer chestnut-brown desk under a more saturated
  ...
  // 'greyscale' (warmer near-black-brown neutrals, light-only) sets — see
  // UI_COLOR_PRESET_SETS.noir's own comment for why splitting near-
  // monochrome presets by an assumed light/dark card never actually
  // matched how they get picked. 8 presets, 4 light + 4 dark, per the
  // project owner's own ask, keeping Charcoal & Birch and Espresso &
  // Cream (both explicitly asked to stay) among the light four. Not just
  // the old 'noir' set trimmed to 4 and the old 'greyscale' trimmed to
  // 4 — Umber & Smoke and Driftwood & Dusk are brand-new dark presets
  // built by carrying the old greyscale's warm-brown (Umber & Pewter's
  ...
  noir: {
    ...
    presets: [
      ...
      // looking like a plain document instead of a ledger.
      ...
      // New: the dark register Umber & Pewter's own warm-brown-neutral
      // mood never had a counterpart for — every other dark neutral here
      // (and in the old 'noir' set it replaces most of) is a cooler true
      // black/grey.
      { id:'umbersmoke',    label:'Umber & Smoke',    bg:'#130F0B', paper:'#241D16' },
      // New: Driftwood & Fog's own "real *mid*-toned desk, closer in
      // lightness to its own paper than to any other entry's bg" outlier
      // idea, carried into the dark register — bg and paper sit much
      // closer together here than any other dark preset in the app,
      // still both comfortably under the 0.5 luminance line.
      { id:'driftwooddusk', label:'Driftwood & Dusk', bg:'#3A342C', paper:'#453E33' }
    ]
  }
};
...
// Used by applyThemeObject()'s dark-mode inkFromUi branch: starting from
// startV, nudges an HSV color's value up in small steps until its actual
// rendered luminance clears bgLum (a relLuminance() reading) by
// targetContrast, or ceil is reached — whichever comes first. Deliberately
// directional (fg luminance vs. bg, not the symmetric max/min WCAG
// formula) — the symmetric version can't tell "brighter than the
// background" from "darker than it by the same ratio," so a candidate
// that had drifted darker than an already-fairly-dark paper could pass
// undetected, rendering as a near-black smudge rather than legible light
// text. Also why this exists instead of a flat value floor: HSV value
// alone doesn't track perceived brightness across hues (relLuminance()
// weights green ~10x more than blue), so a flat floor still left some
// hues reading as barely-there against a very dark paper even once
// "floored" by V alone. A source already above the target needs no lift
// at all, which is what lets an already-legible color keep its own
// natural darkness (the "moody colored text" ask) instead of every
// dark-mode combination getting flattened to one same bright tone.
function liftValueForContrast(h, s, startV, bgLum, targetContrast, ceil){
  let v = Math.max(0, Math.min(startV, ceil));
  for(let i=0; i<40; i++){
    const lum = relLuminance(hsvToHex(h, s, v));
    if((lum + 0.05) / (bgLum + 0.05) >= targetContrast || v >= ceil) break;
    v = Math.min(v + 0.02, ceil);
  }
  return v;
}
...
function applyThemeObject(t){
  ...
  // Text/icon color for the handful of elements whose own background IS
  // --primary (the location badge, the quick-add "+" button) — same
  // luminance-of-the-actual-background idiom the colored tab bar's own
  // label color already uses (relLuminance(cat.hex) in tabSubtagHtml()/
  // renderTabs(), 06-tabs-render.js), not --ink. These used to just read
  // --ink directly, which happened to work by coincidence (light mode's
  // ink is a fixed dark neutral, dark mode's a fixed cream, and most
  // accent colors are mid-toned enough that either reads fine against
  // them) right up until inkFromUi started deriving --ink FROM this same
  // primary color — at that point the badge's text and its own background
  // could land on the same hue, disappearing into it. Deriving this
  // separately from the accent's own luminance (regardless of inkFromUi)
  // fixes that at the source rather than special-casing inkFromUi.
  root.setProperty('--on-primary', relLuminance(ui.primary) > 0.5 ? '#2A2318' : '#F1EAD9');
  ...
  //   Originally gated to `!dark` entirely — a dark Ledger's own paper is
  // already so dark that the light-mode formula's low value (0.34) read as
  // "lost," so it silently fell back to the flat cream ink instead. Now
  // handled with its own dark-mode branch below rather than disabled: see
  // its own comment for the value-floor idea that keeps it legible without
  // just forcing every dark-mode combination up to one bright, samey tone.
  const inkFromUi = !!t.inkFromUi;
  if(inkFromUi){
    ...
    if(dark){
      // Dark Ledger: needs the opposite move from light mode — LIGHT
      // enough to read against a dark paper, not light mode's deliberately
      // dark ink. Simply flipping to a high fixed value (mirroring light
      // mode's fixed 0.34) would flatten every dark-mode combination to
      // one same bright tone regardless of the source color's own
      // character — per the explicit ask, a source that's already
      // reasonably light (or a moody-but-legible dark one, see Photo 1)
      // should keep close to its own natural value instead of being
      // pushed all the way up.
      //   "Capped to a certain darkness level" is measured as actual
      // contrast against the paper (relLuminance()), not raw HSV value —
      // value alone doesn't track perceived brightness across hues (a
      // saturated red/brown reads noticeably darker than a yellow/green at
      // the same V, since luminance weights green ~10x more than blue),
      // so an early flat-V-floor version still left some hues reading as
      // barely-there (~2:1 contrast) against a very dark paper even once
      // "floored." liftValueForContrast() below instead nudges V up in
      // small steps until the actual rendered color clears a target
      // contrast ratio, capped at inkValueCeil so it still stops short of
      // full-bright for a source that was already legible from the start.
      // The saturation boost (darkBoost, the "should go up a bit if the
      // UI color is especially dark" ask) is applied first, since raising
      // V afterward without it is what washes a naturally-dark, saturated
      // color out toward gray.
      const inkValueCeil = 0.94, softValueCeil = 0.86;
      const darkBoost = Math.max(0, 0.5 - sourceHsv.v) * 0.6;
      const darkSat = Math.min(tintSat + darkBoost, 0.85);
      const paperLum = relLuminance(t.paper);
      const inkV = liftValueForContrast(sourceHsv.h, darkSat, sourceHsv.v, paperLum, 3.2, inkValueCeil);
      // ink-soft sits a little closer to the (dark) paper than ink itself
      // — same "less prominent, more muted" relationship light mode's own
      // ink/ink-soft pair has, just mirrored since paper's own lightness
      // is at the opposite end here — hence the lower target contrast and
      // a slightly darker starting point.
      const softV = liftValueForContrast(sourceHsv.h, darkSat, sourceHsv.v - 0.12, paperLum, 2.1, softValueCeil);
      const inkHex = hsvToHex(sourceHsv.h, darkSat, inkV);
      root.setProperty('--ink', inkHex);
      root.setProperty('--ink-soft', hsvToHex(sourceHsv.h, darkSat, softV));
      root.setProperty('--line', hexToRgba(inkHex, 0.22));
    } else {
