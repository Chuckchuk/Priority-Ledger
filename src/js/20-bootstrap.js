...
function classifySwipeZone(target){
  ...
  // A touch that starts on a text field ALREADY being edited (it's the
  // live document.activeElement) is trying to reposition the cursor or
  // extend a selection, never to swipe the page away — per the explicit
  // ask, this has to opt out of every swipe zone below (back, day-nav,
  // month-nav), not just the ones that happen to overlap a text field in
  // practice. Scoped to the FOCUSED field specifically, not "any field
  // anywhere" (an earlier version of this guard) — that blocked swiping
  // starting on a title/Notes/Add Step field any time it merely existed
  // on the page, focused or not, which read as a dead zone covering most
  // of a Task Detail page rather than the narrow "don't fight an active
  // text edit" case this was actually meant for. A field that ISN'T
  // focused yet has nothing to reposition — touching it starts a swipe
  // like anywhere else, and only focuses on release; per the explicit
  // ask, this also means a touch elsewhere on the page still swipes fine
  // even while a DIFFERENT field is mid-edit, since only the specific
  // touched element is checked against activeElement, not "is anything
  // being edited at all." Mirrors the identical guard pullRefreshEligible()
  // already has for the same reason. A plain vertical scroll starting
  // here is untouched either way — returning null here means
  // swipeGesture never gets set, so the touchmove listener below bails
  // immediately and never calls preventDefault(), leaving native
  // scroll/selection completely alone.
  const editingField = target.closest('input, textarea, select, [contenteditable="true"]');
  if(editingField && editingField === document.activeElement) return null;
...
function pullRefreshEligible(target){
  ...
  // screen), and never starting on a text field that's ALREADY focused —
  // a drag meant to reposition a cursor or select text shouldn't also arm
  // a page reload. Scoped to the focused field specifically, same fix and
  // same reasoning as classifySwipeZone()'s identical guard above (see
  // its own comment) — an unfocused field has nothing to reposition yet,
  // so a touch starting there is free to arm a pull like anywhere else.
  // Deliberately NOT gated on scrollY here (unlike an earlier version) —
  // see touchmove below.
  const appShell = document.getElementById('appShell');
  if(!appShell || appShell.style.display === 'none') return false;
  const editingField = target.closest && target.closest('input, textarea, select');
  if(editingField && editingField === document.activeElement) return false;
  ...
  // A touch starting on the tab bar itself is trying to tap or (in the
  // 'scroll' mobile style) horizontally scroll through tabs — not to pull
  // the page down. Without this, dragging through #tabs while it's
  // horizontally scrollable also visibly dragged #appShell down toward a
  // refresh at the same time, fighting the tab-bar scroll.
  if(target.closest && target.closest('#tabs')) return false;
  return true;
}
...
document.addEventListener('touchmove', (e) => {
  ...
  // Same reasoning, for the swipe-left row actions (rowSwipeGesture) —
  // those attach their own touchmove directly to the row, which runs
  // first and can lock the gesture onto the x-axis mid-touch, well after
  // this same touch already armed a pull gesture at touchstart (rows
  // don't opt out of pullRefreshEligible() up front, since a touch that
  // ends up scrolling vertically instead should still be able to arm a
  // pull normally). Once it's a confirmed sideways row drag, bail here too.
  if(rowSwipeGesture && rowSwipeGesture.axis === 'x'){
    pullRefreshGesture = null;
    pullRefreshReset();
    return;
  }
